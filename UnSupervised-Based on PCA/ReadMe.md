...
- 异常样本具有**few and different**的特点，即异常样本占比较少，且特征构成与正常样本不一致。

- 根据特征值的大小，对主成分降序排列，则靠前的主成分解释了大部分正常样本的方差，而最后的主成分主要解释了异常样本的方差。
...
- 若只选取靠前的主成分用于重构初始特征空间，则异常样本无法被完整表出，从而使得异常样本带来的重构误差要远高于正常样本。因此，**若只运用靠前的主成分进行矩阵重构，则重构误差越高的样本越有可能是异常样本**
