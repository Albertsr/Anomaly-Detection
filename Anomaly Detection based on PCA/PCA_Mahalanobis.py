...
class PCA_Anomaly(PCA):
    ...
    def anomaly_score(self):
        projection_matrix, ev = self.eig()[0], self.eig()[1]
        def score(idx):
            inner_product = np.dot(self.scale(), projection_matrix[:, idx])
            score = np.square(inner_product) / ev[idx]
            return score
        scores = sum(list(map(score, range(len(ev)))))
        return scores
